...
class Tester(object):
    def __init__(self, dataset_path='', model_path='', used_model='',
        ...
        # load model
        self.args = self.set_config(self.args)
        self.test_loader, self.model = self.set_environment(self.args)

    def set_config(self,args):
        args.val_root = self.dataset_path+'/test/images/'
        args.num_classes = len(os.listdir(args.val_root))
        args.device =self.device
        args.pretrained_path = self.model_load_path
    ...
    def test(self):
        ...
        self.test_metric_info = {}
        # test
        acc = self.test_epoch(self.args, self.model,self.test_loader)
        self.test_metric_info['acc'] = round(acc, KEEP_DIGITS_NUM)
    ...
    def test_epoch(self,args, model, test_loader):
        total = 0
