...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def getDataInfo(self, root):
        ...
        cla_dict = dict((key,val) for key, val in enumerate(folders))
        json_str = json.dumps(cla_dict, indent=1)
        with open('/app/tianji/class_indices.json', 'w') as json_file:
            json_file.write(json_str)
        print('class dict save to class_indices.json')
